...
import pyglet
from experimental.dist_field.genfield import gen_dist_field
...
//float screenPxRange() {
//    vec2 unitRange = vec2(pxRange)/vec2(textureSize(text, 0));
//    vec2 screenTexSize = vec2(1.0)/fwidth(texture_coords);
...
class SDFTextLayoutGroup(graphics.Group):
    ...
    def __init__(self, layout: SDFTextLayout, texture: Texture, program: ShaderProgram, order: int = 1,  # noqa: D107
...
class _SDFGlyphBox(base._GlyphBox):
    owner: Texture
    font: Font
    ...
    def place(self, layout: SDFTextLayout, i: int, x: float, y: float, z: float, line_x: float, line_y: float,
        ...
        try:
            ...
            group = layout.group_class(layout, self.owner, layout.program, order=1, parent=layout.group)
...
class SDFTextLayout(base.TextLayout):
    ...
    group_class: ClassVar[type[TextLayoutGroup]] = SDFTextLayoutGroup
    ...
    def __init__(self, document: AbstractDocument,
                 ...
                 wrap_lines: bool = True, init_document: bool = True, sdf_font = None) -> None:
        ...
        print("SDFTextLayout", SDFTextLayout)
    ...
    def _get_glyphs(self) -> list[_InlineElementBox | Glyph]:
        ...
        for start, end, (font, element) in runs.ranges(0, len(text)):
            if element:
                ...
                glyphs.extend(self.sdf_font.get_glyphs(text[start:end]))
    ...
    def _flow_glyphs_wrap(self, glyphs: list[_InlineElementBox | Glyph], owner_runs: runlist.RunList, start: int,
        ...
        for start, end, owner in owner_iterator:
                ...
                if wrap != "char" and text in "\u0020\u200b\t":
                    ...
                    if (wrap and self._wrap_lines and x + kern + glyph.advance >= width) or new_line:
                        ...
                        if owner_accum_commit:
                            line.add_box(_SDFGlyphBox(owner, font, owner_accum_commit, owner_accum_commit_width))
                            owner_accum_commit = []
                            owner_accum_commit_width = 0
            ...
            if owner_accum_commit:
                line.add_box(_SDFGlyphBox(owner, font, owner_accum_commit, owner_accum_commit_width))
            if owner_accum:
                run_accum.append(_SDFGlyphBox(owner, font, owner_accum, owner_accum_width))
    ...
    def _flow_glyphs_single_line(self, glyphs: list[_InlineElementBox | Glyph], owner_runs: runlist.RunList,
        ...
        for start, end, owner in owner_iterator:
            ...
            if owner is None:
                ...
                line.add_box(_SDFGlyphBox(owner, font, owner_glyphs, width))
