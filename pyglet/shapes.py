...
def _point_in_polygon(polygon: Sequence[tuple[float, float]], point: tuple[float, float]) -> bool:
    """Use ray casting to determine if a point is inside a polygon, including its edge.

    Based on the MIT-licensed example at:
    https://www.algorithms-and-technologies.com/point_in_polygon/python

    The iteration was corrected to visit every edge consistently and to
    consider a point on an edge to be inside the polygon.
    """
    inside = False
    px, py = point
    for i, current in enumerate(polygon):
        ...
        cross = (px - previous[0]) * dy - (py - previous[1]) * dx
        if abs(cross) <= 1e-9 and min(previous[0], current[0]) <= px <= max(previous[0], current[0]) and \
                min(previous[1], current[1]) <= py <= max(previous[1], current[1]):
            return True
        if (current[1] > py) != (previous[1] > py):
            x_intersection = previous[0] + (py - previous[1]) * dx / dy
