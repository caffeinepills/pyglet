...
class PlayerTestCase(unittest.TestCase):
    ...
    def assert_new_texture_created(self, video_format):
        self.mock_texture_create.assert_called_once_with(video_format.width, video_format.height)
...
class SourcePlayTestCase(unittest.TestCase):
    def test_eos_releases_fire_and_forget_player(self):
        ...
        self.assertTrue(player._event_stack)
    ...
    def test_overriding_eos_handler_does_not_prevent_cleanup(self):
        """The user-facing EOS callback cannot replace fire-and-forget cleanup."""
        source = SeekableSource()
        source.video_format = VideoFormat(1, 1)
        player = source.play()
        eos_callbacks = []
